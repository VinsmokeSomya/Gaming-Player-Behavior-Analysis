...
def generate_sample_funnel_data() -> pd.DataFrame:
    ...
    base_players = 100000
    drop_off_rates = [0, 0.15, 0.25, 0.10, 0.15, 0.20, 0.25, 0.30, 0.60]
    
    data = []
    current_players = base_players
    
    for i, (stage, drop_rate) in enumerate(zip(stages, drop_off_rates)):
        if i > 0:
            current_players = int(current_players * (1 - drop_rate))
...
def generate_sample_level_data() -> pd.DataFrame:
    """Generate sample level completion data for heatmap."""
    level_groups = ['Tutorial', 'Beginner', 'Intermediate', 'Advanced', 'Expert']
    levels_per_group = 10
    
    data = []
    for group_idx, group in enumerate(level_groups):
        base_completion = 0.9 - (group_idx * 0.15)  # Decreasing completion rates
        
        for level in range(1, levels_per_group + 1):
            # Add some variation within each group
            level_difficulty = (level - 1) / levels_per_group
            completion_rate = base_completion * (1 - level_difficulty * 0.3)
            completion_rate = max(0.1, completion_rate)  # Minimum 10% completion
...
def generate_sample_cohort_funnel_data() -> pd.DataFrame:
    """Generate sample cohort funnel comparison data."""
    stages = ["Tutorial", "Level 5", "Level 10", "Level 20", "First Purchase"]
    cohorts = ["Week 1", "Week 2", "Week 3", "Week 4"]
    
    data = []
    for cohort_idx, cohort in enumerate(cohorts):
        base_conversion = 1.0
        cohort_performance = 1.0 - (cohort_idx * 0.05)  # Slight degradation over time
        
        for stage_idx, stage in enumerate(stages):
            # Simulate funnel drop-off
            stage_conversion = base_conversion * (0.8 ** stage_idx) * cohort_performance
